...
const RecordModal = ({ open, onClose, record, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) await updateRecord(record._id, payload);
    ...
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Record' : 'New Record'}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <Input label="Amount (₹)" type="number" step="0.01" min="0" placeholder="0.00" {...f('amount')} />
        ...
        <Input label="Category" placeholder="e.g. salary, rent, food" {...f('category')} />
