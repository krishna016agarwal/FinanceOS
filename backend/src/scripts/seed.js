...
const seed = async () => {
  ...
  // Replace the record generation loop in seed.js
  ...
  for (let i = 0; i < 150; i++) {
    ...
    const amount =
      type === "INCOME"
        ? randomInt(5000, 200000) * 100
        : randomInt(500, 80000) * 100;

    // Weighted date distribution — more recent records are more likely
    // 50% of records in last 30 days
    // 30% of records in last 31-90 days
    // 20% of records in last 91-365 days
    let daysAgo;
    const rand = Math.random();
    if (rand < 0.5) daysAgo = randomInt(0, 30);
    else if (rand < 0.8) daysAgo = randomInt(31, 90);
    else daysAgo = randomInt(91, 365);
  ...
  // Sort records by date descending before inserting
  // This also makes createdAt roughly match date order
  records.sort((a, b) => b.date - a.date);

  // Soft delete 5 oldest records — makes more sense than deleting recent ones
  records.slice(145, 150).forEach((r) => {
    r.isDeleted = true;
  ...
  console.log(
    `  Viewers        : ${users.filter((u) => u.role === "VIEWER").length} (1 inactive)`,
  );
